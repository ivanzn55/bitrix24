...
namespace Otus\Crmtab\Handlers;
...
class CrmTab
{
    ...
    {
        $availableEntityIds = Option::get('otus.crmtab', 'ENTITIES_TO_DISPLAY_TAB');
        ...
        if (in_array($entityTypeId, $availableEntityIds)) {
            $tabs[] = [
                ...
                'name' => Loc::getMessage('OTUS_CRMTAB_TAB_TITLE'),
                'enabled' => true,
                'loader' => [
                    'serviceUrl' => sprintf(
                        '/bitrix/components/otus.crmtab/task.grid/lazyload.ajax.php?site=%s&%s',
                        \SITE_ID,
                        \bitrix_sessid_get(),
                    ),
                    'componentData' => [
